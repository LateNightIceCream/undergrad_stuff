...
totaldata    <- data.frame(x = datat$f, y=datat$delta)

ylabel <- bquote( delta / ~ degree )
xlabel <- bquote( "f" / ~"Hz")
...
pdf("3_3_Verlustwinkel.pdf", width = output_width, height = output_height, )

####################################################################

p9 <- ggplot(totaldata, aes(datat$f, datat$delta)) +
