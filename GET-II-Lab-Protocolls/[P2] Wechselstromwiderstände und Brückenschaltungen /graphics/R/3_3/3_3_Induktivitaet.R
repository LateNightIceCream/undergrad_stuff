...
totaldata    <- data.frame(x = datat$f, y=datat$L)
...
pdf("3_3_Induktivitaet.pdf", width = output_width, height = output_height, )

####################################################################

p9 <- ggplot(totaldata, aes(datat$f, datat$L)) +
        ...
        geom_smooth(se=FALSE, color="grey68") +
