...
xbreaks <- seq(-4,4,1)
xbreaks <- c(xbreaks, 1.5, -1.5)
#xlabels <- c("","",bquote("-"~omega[g]),"",0,"",bquote(omega[g]),"","")
xlabels <- c("","",bquote("-2"~omega["0"]),bquote("-"~omega["0"]),0,bquote(omega["0"]),bquote("2"~omega["0"]),"","",bquote(omega["g"]),bquote(-omega[g]))
...
p <- ggplot(data.frame(x=c(0,2)), aes(x=x)) +
    ...
    geom_segment( x = -1.5, y=0, xend=-1.5, yend=0.5, color=tpcolor, linetype="dashed" ) +
    geom_segment( x = 1.5, y=0, xend=1.5, yend=0.5, color=tpcolor, linetype="dashed" ) +
    geom_segment( x = 1.5, y=0.5, xend=-1.5, yend=0.5, color=tpcolor, linetype="dashed" ) +
    geom_segment( x = -1.5, y=0, xend=-4, yend=0, color=tpcolor, linetype="dashed" ) +
    geom_segment( x = 1.5, y=0, xend=4, yend=0, color=tpcolor, linetype="dashed" ) +
