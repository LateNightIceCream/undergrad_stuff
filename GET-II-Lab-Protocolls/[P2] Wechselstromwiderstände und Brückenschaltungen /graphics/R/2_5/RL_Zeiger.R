...
pdf("RL_Zeiger.pdf", width = output_width, height = output_height)
...
p9 <- ggplot(data.frame(x=c(0, 2),y=c(0,2)), aes(x,y)) +
        ...
        ggtitle("Zeigerbild der Impedanz einer verlustbehafteten Induktivität / Spule im Serienmodell") +
        ...
        scale_y_continuous(limits=c(0, 1.382),  breaks = c(0,1), labels=c( 0, bquote(omega ~ "L"["s"]) ) ) +
        scale_x_continuous(limits=c(0, 1.382),  breaks = c(0,Rpos), labels=c(0, bquote("R"["sL"]) ) )+

        #lines "#E58700"
        # L
        geom_segment( aes(x = 0, y = -1, xend = Rpos, yend = -1 ), color=Lcolor,   linetype="dashed", size = 1.618)+
        # R
        geom_segment( aes(x = Rpos, y = 0, xend = Rpos, yend = -1 ), color=Rcolor,   linetype="dashed", size = 1.618) +
        # total
