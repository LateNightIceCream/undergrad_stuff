...
pdf("ESBp_Kondensator.pdf", width = output_width, height = output_height)
...
p9 <- ggplot(data.frame(x=c(0, 2),y=c(0,2)), aes(x,y)) +
        ...
        scale_y_continuous(limits=c(0, 1.23606797749978964),  breaks = c(0,1), labels=c( "",bquote(omega ~ "C"["p"])) ) +
        scale_x_continuous(limits=c(0, 1.23606797749978964),  breaks = c(0,Rpos), labels=c("", bquote(over(1,"R"["pC"]))) )+
