...
pdf("RL_Zeiger_parallel.pdf", width = output_width, height = output_height)
...
p9 <- ggplot(data.frame(x=c(0, 2),y=c(0,2)), aes(x,y)) +
        ...
        ggtitle("Zeigerbild der Admittanz einer verlustbehafteten Induktivität / Spule im Parallelmodell") +
        ...
        scale_y_continuous(limits=c(-1.382, 0),  breaks = c(0,-1), labels=c( 0, bquote("-"~over(1,omega ~ "L"["p"])) ) ) +
        scale_x_continuous(limits=c(0, 1.382),  breaks = c(0,Rpos), labels=c(0, bquote(over(1,"R"["pL"]))), position="top")+
